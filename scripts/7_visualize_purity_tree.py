...
def build_dot(root, max_depth: int, variant: str):
    ...
    Each edge label is the child node's n_docs (subset size).
    """
    ...
    def visit(node, parent_id=None, edge_label=None):
        ...
        # if you ever want more info in the box, uncomment this:
        ...
        # draw edge from parent with label = child subset size
        ...
        # recurse, labeling edges by child n_docs
        if left is not None:
            left_docs = left.get("n_docs", 0)
            visit(left, parent_id=my_id, edge_label=str(left_docs))

        if right is not None:
            right_docs = right.get("n_docs", 0)
            visit(right, parent_id=my_id, edge_label=str(right_docs))
